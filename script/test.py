...
def snpDict(x):
    ...
    if multi_check == True:
        ...
        snp_dict['varify_allele'] = alt_dict.get(current_pos) if alt_dict.get(current_pos) != None else x.ref_allele # fill with current pos snp
...
def main():
    with concurrent.futures.ProcessPoolExecutor(num_processes) as pool:
        ...
        x['varify_allele'] = list(tqdm.tqdm(pool.map(getAlt2, x.pileup, chunksize=10), total=x.shape[0]))
        mp['varify_allele'] = list(tqdm.tqdm(pool.map(getAlt2, mp.pileup, chunksize=10), total=x.shape[0]))

    return x, mp

# ENCFF283TLK_chr12.mpileup
mp = readMP('/scratch/kk4764/VARify/data/test/ENCFF283TLK_chr12.mpileup')
snptable = pd.read_table('/scratch/kk4764/VARify/data/test/ENCFF541HLI.snp_effects.chr12_only_filtergenic_snps.no_intron.no_utr.tsv')
...
def main2():
    with concurrent.futures.ProcessPoolExecutor(num_processes) as pool:
        ...
        print('translating...')
        df['varify_aa'] = list(tqdm.tqdm(pool.map(translate, df.varify_codon, chunksize=10), total=df.shape[0]))

        print('alt validating...')
        df['nt_VARified'] = list(tqdm.tqdm(pool.map(validate, df.alt_allele, df.varify_allele, chunksize=10), total=df.shape[0]))

        print('codon validating...')
        df['codon_VARified'] = list(tqdm.tqdm(pool.map(validate, df.varify_codon, df.alt_codon, chunksize=10), total=df.shape[0]))
...
if __name__ == '__main__':
    ...
    with open('alt_dict.pkl', 'wb') as ad:
        pickle.dump(alt_dict, ad)
        print('Alt_Dict saved to file alt_dict.pkl')

    # alt_dict.to_csv('/scratch/kk4764/VARify/alt_dict.csv')

    # df['result'] = list(tqdm.tqdm(pool.map(func, df['a'], df['b'], chunksize=10), total=df.shape[0]))  # With a progressbar

    # running the script
    # snp_df = tqdm.tqdm(pool.map(snpDict, x, chunksize=10), total=x.shape[0])
